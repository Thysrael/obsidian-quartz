...
- 椭圆曲线：椭圆曲线通用方程为 $y^{2} = x^{3} + ax + b$ 任何一条不垂直的直线与曲线的交点不超过 3 个。定义一种运算，曲线上两个点为 $A, B$，$AB$ 连直线得到 $C^{' }$ ，$C^{' }$ 关于 x 轴有对称点 $C$ ，称作 $A \space \cdot \space B = C$ 。给定一个初始点 $A$ ，一个动点 $B$ ，经过 $n$ 次 $\cdot$ 运算之后得出 $Z$ ，从 $A, Z$ 求出 $n$ 比较困难。
