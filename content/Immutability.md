...
但这种错误的观点，是我把“编程模型”和“硬件实现”搞混了。所谓的不可变性，只是在编程模型上，可能会导致数据的复制，而落实到具体硬件上面，可能还是同一份内存上数据内容的修改，只要不影响上层抽象就可以了。

说白了，不可变性是一种数据流的表达方式（见 [[Dataflow vs State]]），每个变量不再是一个“内存格子”，而是 DAG 图上的一个节点。
