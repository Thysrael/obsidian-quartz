...
A 和 B 无法阻止 X 监听信道，所以选择不传递明文信息，而是选择对信息进行 [[Symmetric Encryption]]，在信道中传递密文，这样 X 监听到的都是密文，因为 X 不具被密钥，所以无法解密。
...
1.  使用 [[Key Exchange]] ，这个一个数学算法，可以保证在被监听的信道上可靠得传递对称加密的密钥
2.  使用 [[Asymmetric Encryption]] ，这是一种新型加密技术，这种技术的特性不介意信道中的密钥被监听
...
为了确定密钥的来源，A 和 B 采用了 [[Digital Certificate]] 技术，这种技术利用了一个完全可信的第三方公钥，保证了 A 和 B 密文来源的可信性。
...
为了保证密文完整性，A 和 B 采用了 [[Digital Signature]] 技术，这种技术会在加密明文的同时加密明文的摘要（摘要就是根据明文生成的一个哈希），这样如果 X 直接篡改明文，就会导致明文和摘要不匹配。
...
为了应对 X 的量子计算机破解法，A 和 B 使用了 [[Quantum Safety]] 。
